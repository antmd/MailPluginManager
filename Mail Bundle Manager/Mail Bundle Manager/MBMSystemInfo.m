...
+ (MBMSystemInfo *)sharedInstance;
@end

@implementation MBMSystemInfo
...
- (NSDictionary *)completeInfo {
	if (_completeInfo == nil) {
		...
		for (MBMMailBundle *aBundle in bundles) {
		...
		bundles = [MBMMailBundle allDisabledMailBundlesShouldLoadInfo:NO];
...
+ (MBMSystemInfo *)sharedInstance {
	...
	static MBMSystemInfo	*sharedHelper;
